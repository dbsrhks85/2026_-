...
INSERT INTO departments (key, label, color, keywords, tasks)
VALUES
('road', '도로과', '#ef4444', ARRAY['도로', '포트홀', '아스팔트', '차선', '보도블럭', '인도', '신호등'], ARRAY['현장 위치 확인', '도로 파손 여부 점검', '긴급 보수 필요성 판단', '보수 일정 등록']),
...
('environment', '환경과', '#14b8a6', ARRAY['쓰레기', '악취', '폐기물', '소음', '먼지', '오염'], ARRAY['환경 피해 여부 조사', '현장 측정 요청', '정화/수거 조치 요청', '재발 방지 검토']),
('planning', '기획예산과', '#a855f7', ARRAY['정책', '예산', '개선', '제안', '건의'], ARRAY['정책 제안 검토', '예산 반영 가능성 검토', '중장기 과제 분류', '부서 협의 요청']),
('civil', '민원담당관', '#64748b', ARRAY[]::TEXT[], ARRAY['민원 내용 1차 검토', '소관 부서 재배정', '처리 기한 모니터링', '민원인 회신 관리'])
ON CONFLICT (key) DO UPDATE SET
    label = EXCLUDED.label, color = EXCLUDED.color, keywords = EXCLUDED.keywords, tasks = EXCLUDED.tasks;
"""
